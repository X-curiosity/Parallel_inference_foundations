...
x - hx <= i <= x + hx,
y - hy <= j <= y + hy.
This is the sliding window. Note that the window will contain at most (2*hx+1) * (2*hy+1) pixels, 
but near the boundaries there will be fewer pixels.
  ...
  for(int x = 0; x < nx; x++){
    for(int y = 0; y < ny; y++){
     ...
     // Filling in the sliding window
      ...
      std::sort(window.begin(), window.begin() + count); // Sorting the matrix/vector
